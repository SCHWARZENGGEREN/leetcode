...
public class FrogJump {
    ...
    public static boolean canCross(int[] stones) {
        ...
        return jump(stones, 1, 1) || jump(stones, 1, 2);
    }


    public static boolean jump(int[] stones, int idx, int k) {
        if (idx >= stones.length - 1) return true;
        if (stones[idx] - stones[idx - 1] > k) return false;
        if (idx == 4){
            System.out.println();
        }

        return jump(stones, idx + k - 1, k - 1) ||
                jump(stones, idx + k, k) ||
                jump(stones, idx + k + 1, k + 1);
    }
